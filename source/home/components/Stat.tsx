...
type FullAttributeType = 'hp' | 'attack' | 'defense' | 'special-attack' | 'special-defense' | 'speed'
...
export default function Stat({ name, baseStat, type }: StatType) {
  ...
  const [, shortName] = dictionaryStat.find(([item,]) => item === name) as ShortStatType
  ...
  return (
    <View style={styles.stat}>
      ...
      <Text style={styles.numberText}>{baseStat > 99 ? baseStat : '0' + baseStat}</Text>
